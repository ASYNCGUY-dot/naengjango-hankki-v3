...
**메시지의 "30초"는 실측 22.9초보다 넉넉하게 잡은 값이다.** 일부러 그렇게 뒀다. 예상보다
빨리 뜨면 아무도 불평하지 않지만, 예상보다 늦으면 고장으로 읽힌다. Render 자체 안내도
"50초 이상 걸릴 수 있다"고 하므로 회차별 편차도 있다.
...
> `usage_events`는 2026-08-18에 비웠다. 개발 중 검증으로 쌓인 비로그인 열람 10행이
> 남아 있었는데, 지우고 시작하는 편이 날짜로 걸러내는 것을 매번 기억하는 것보다 낫다.
> **지금 이 표에 나오는 것은 전부 지인이 실제로 한 행동이다.**
