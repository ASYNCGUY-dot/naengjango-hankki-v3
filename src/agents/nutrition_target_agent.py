...
# 화면이 사용자에게 보여줄 선택지. 이 파일이 원본이다.
#
# 2026-08-18에 실제로 어긋났다. 가입 화면이 "50대"와 "60대 이상"을 주고 있었는데 RDA_TABLE은
# "50대 이상"만 알아서, 그 둘을 고른 사용자는 영양 기준을 하나도 못 받았다. 화면이 선택지를
# 따로 들고 있으면 이런 어긋남이 조용히 생긴다 - 알레르기에서 겪은 것과 같은 구조라
# 같은 방식(서버가 목록을 내려준다)으로 막는다.
AGE_GROUP_OPTIONS = list(AGE_GROUP_TO_BRACKET_LABEL)

# RDA 표에 기준이 있는 성별. 이 둘만 영양 기준을 계산할 수 있다.
GENDERS_WITH_TARGETS = ["여성", "남성"]

# 성별은 밝히지 않을 수 있어야 한다. 건강 정보를 강제로 받지 않는다는 원칙이고,
# 이 값을 고르면 영양 기준만 못 보여줄 뿐 나머지 기능은 그대로 된다.
GENDER_UNDISCLOSED = "선택 안 함"
GENDER_OPTIONS = [*GENDERS_WITH_TARGETS, GENDER_UNDISCLOSED]
