...
--
-- 주의: 되돌릴 수 없다. 2026-08-18 기준으로 계정 44개와 그에 딸린 데이터를 지우고
-- 관리자(최지수) 한 행만 남는다.
--
-- 백업은 Supabase 대시보드에서 받을 수 없다. 무료 플랜에는 백업 기능이 없다
-- ("Free Plan does not include project backups"). 대신 이 저장소의
-- scripts/backup_before_005.py가 이 파일이 지울 범위만 골라 INSERT 문으로 떠준다.
-- 적용 전에 그것부터 실행할 것.
...
WHERE username IS NULL
   ...
   OR username IN (
        ...
        'e2e_verify_20260813',
        -- 사용 로그(007)가 실제로 쌓이는지 확인하려고 만든 계정(2026-08-18).
        -- 남겨두면 Phase 4 집계에 개발자 활동이 섞인다. usage_events는 CASCADE로 함께 간다.
        'e2e_usage_20260818',
        -- 배포된 화면으로 가입·온보딩·초기화를 끝까지 태워본 계정(2026-08-18).
        -- 최지수 님 실제 이메일을 물고 있으므로, 남겨두면 나중에 그 주소로 다시 가입할 수 없다
        -- (users.email이 UNIQUE다).
        'e2e_deploy_20260818'
      );
